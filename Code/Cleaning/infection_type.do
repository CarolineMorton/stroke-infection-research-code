use lrti_all, clear
duplicates drop patid lrti_eventdate , force /*remove duplicates (same patient, same day)*/
sort patid lrti_eventdate
...
drop if lrti_eventdate > end
drop if lrti_eventdate < str_date
keep patid lrti lrti_eventdate 
...
drop littlen start end
rename hospitalisation_eventdate eventdate
rename HES_infection infectionType
replace infectionType=4 if infectionType==1
save hosp_multi, replace

use lrti_multi, clear
append using uti_multi
append using ssti_multi
append using hosp_multi
lab def type1 1 "LRTI" 2 "UTI" 3 "SSTI" 4 "Hospitalisation"
lab val infectionType type1
sort patid (eventdate)
gen start=eventdate
gen end=eventdate+27 /*ADD 3 months to end of Rx*/
format start end %td
bysort patid (eventdate): gen littlen=_n
*flag infections in same episode -  give them the same littlen value
by patid: replace littlen=littlen[littlen-1] if start<end[_n-1]  & _n!=1
*collapse infections with overlapping dates: take earliest start and latest end date
collapse (min) start (max) end (min) eventdate, by(patid littlen infectionType)  
drop littlen start end
duplicates drop patid eventdate, force /*remove duplicates (same patient, same day)*/
sort patid eventdate
save infection_multi2, replace

use infection_multi2, clear
*Find the infections in study period
merge m:1 patid using post_stroke_dementia_dataset_v5, keep(match) nogen /*196406*/

drop if eventdate > end
drop if eventdate < str_date /*50429*/
...
save infection_collapsed2, replace

use post_stroke_dementia_dataset_v5, clear
merge 1:1 patid using infection_collapsed2
drop _m
...
save post_stroke_dementia_dataset_multi2, replace
